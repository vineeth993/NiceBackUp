...
class HsnReport(report_xls):
	...
	def monthly_hsn_summary(self, invoice_obj, cr, uid, wb, data):
		...
		for hsn in hsn_dict:
			...
			ws.write(count, 6, hsn_dict[hsn][4], self.number2d)
			ws.write(count, 7, hsn_dict[hsn][2], self.number2d)
			ws.write(count, 8, hsn_dict[hsn][3], self.number2d)
			ws.write(count, 9, hsn_dict[hsn][5], self.number2d)
			count += 1
		...
		ws.write(2, 4, round(hsn_total_amount, 2), self.number2d)
		ws.write(2, 5, round(hsn_total_taxable, 2), self.number2d)
		ws.write(2, 6, round(hsn_total_igst, 2), self.number2d)
		ws.write(2, 7, round(hsn_total_cgst, 2), self.number2d)
		ws.write(2, 8, round(hsn_total_sgst, 2), self.number2d)
		ws.write(2, 9, round(hsn_total_cess, 2), self.number2d)
