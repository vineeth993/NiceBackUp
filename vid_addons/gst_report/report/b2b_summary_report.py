...
class B2bSummary(report_xls):
    ...
    def b2b_sale_summary(self, invoice_obj, cr, uid, wb, data):
        ...
        headers = {0:"No of Recipients", 1:"No Of Invoices", 3:"Total Invoice Value", 9:'Total Taxable Value', 10:'Total Cess'}
        ...
        ws.write(2, 9, total_taxable_amt, self.number)
        ws.write(2, 10, 0, self.number)
    ...
    def b2c_sale_summary(self, invoice_obj, cr, uid, wb, data):
        ...
        headers_s = {0:"Type", 1:"Place Of Supply", 2:'Rate', 3:'Taxable Value', 4:'Cess Amount', 5:'E-Commerce GSTIN'}
        ...
        for invoice in invoices:
            ...
            date_invoice_obj = dt.strptime(str(invoice.date_invoice), '%Y-%m-%d')
            date_invoice = date_invoice_obj.strftime('%d-%b-%Y')
            if invoice.amount_total > 250000 and invoice.sale_sub_type_id.tax_categ == "igst":
                total_invoice_l += 1
                total_inv_amt_l += invoice.amount_total
                for tax in taxes:
                ...
                for tax in taxes:
                    ...
                    wss.write(count_s, 2, tax, self.number)
                    wss.write(count_s, 3, taxes[tax], self.number)
                    wss.write(count_s, 4, 0, self.number)
                    count_s += 1

        headers_l = {0:"No. of Invoices ", 2:'Total Invoice Value', 5:'Total Taxable Value', 6:'Total Cess'}
        headers_s = {3:'Total Taxable Value', 4:'Total Cess'}
        for header in headers_l:
        ...
        wss.write(2, 3, total_taxable_amt_s, self.number)
        wss.write(2, 4, 0, self.number)
