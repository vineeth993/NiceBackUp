		...
		for invoice in self.invoices_id:
			...
			for line in invoice.invoice_line:
				...
				if not item_list.has_key(line.product_id.hs_code_id.code[0:2]):
					item_list.update({line.product_id.hs_code_id.code[0:2]:{round(tax_percnt, 2):[round(hsn_total_taxablevalue, 2), round(hsn_igst_total, 2), round(hsn_sgst_total, 2), round(hsn_cgst_total, 2), round(hsn_cess_total, 2), line.quantity, val]}})
				else:
					if not item_list[line.product_id.hs_code_id.code[0:2]].has_key(round(tax_percnt, 2)):
						item_list[line.product_id.hs_code_id.code[0:2]].update({round(tax_percnt, 2):[round(hsn_total_taxablevalue, 2), round(hsn_igst_total, 2), round(hsn_sgst_total, 2), round(hsn_cgst_total, 2), round(hsn_cess_total, 2), line.quantity, val]})
					else:
