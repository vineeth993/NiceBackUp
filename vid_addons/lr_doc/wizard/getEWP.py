			...
			for line in invoice.invoice_line:
				...
				if not item_list.has_key(product_hsn):
					...
					if not item_list[product_hsn].has_key(round(tax_percnt, 2)):
						item_list[product_hsn].update({round(tax_percnt, 2):[round(hsn_total_taxablevalue, 2), round(hsn_igst_total, 2), round(hsn_sgst_total, 2), round(hsn_cgst_total, 2), round(hsn_cess_total, 2), line.quantity, val, line.product_id.hs_code_id.description]})
					else:
						...
						item_list[product_hsn][round(tax_percnt, 2)][5] += line.quantity
