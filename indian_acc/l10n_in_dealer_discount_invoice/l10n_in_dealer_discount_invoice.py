...
class account_invoice_line(osv.Model):
    ...
    def product_id_change(self, product, uom_id, qty=0, name='', type='out_invoice', partner_id=False, fposition_id=False, price_unit=False, currency_id=False, company_id=None):
        res = {'value':{}}
        if product:
            res = super(account_invoice_line, self).product_id_change(product, uom_id, qty, name, type, partner_id=partner_id, fposition_id=fposition_id, price_unit=price_unit, currency_id=currency_id, company_id=company_id)
            
            pricelist_pool = self.pool.get('product.pricelist')
            
            dealer_id = self._context.get('dealer_id')
            dealer_pricelist_id = self._context.get('dealer_pricelist_id')
            
            if dealer_id and dealer_pricelist_id:
                dealer_res = pricelist_pool.price_get(self._cr, self._uid, [dealer_pricelist_id], product, qty, dealer_id)
